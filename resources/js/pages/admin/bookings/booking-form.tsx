...
import { store, update } from "@/actions/App/Http/Controllers/Admin/BookingController";
...
export default function BookingForm({
    ...
    const selectedPackage = packages.find((packageData) => packageData.id.toString() === packageId);
                ...
                <>
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Pilih customer, paket, tanggal berangkat, dan jumlah peserta. Total
                                dihitung oleh server.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="grid gap-5">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    >
                                        ...
                                        {packages.map((packageData) => (
                                            <option key={packageData.id} value={packageData.id}>
                                                {packageData.title} — {packageData.destination}
                                            </option>
                                        ))}
                                    </select>
                            ...
                            <div className="bg-muted rounded-lg p-4">
                                <p className="text-muted-foreground text-sm">Estimasi total</p>
                                <p className="text-xl font-semibold">
                                    {formatCurrency(estimatedTotal)}
                                </p>
                                <p className="text-muted-foreground text-xs">
                                    {formatCurrency(Number(selectedPackage?.price ?? 0))} ×{" "}
                    ...
                    <div className="flex gap-3">
                        <Button
                            disabled={processing || customers.length === 0 || packages.length === 0}
                        >
                            {processing
                                ? "Menyimpan..."
                                : booking
