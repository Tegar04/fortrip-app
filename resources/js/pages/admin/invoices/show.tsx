...
export default function ShowInvoice({ invoice }: { invoice: InvoiceData }) {
    ...
    function removePayment(paymentId: number) {
        if (window.confirm('Hapus catatan pembayaran ini?'))
            router.delete(destroyPayment.url([invoice.id, paymentId]), {
                preserveScroll: true,
            });
    }
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                    ...
                    <Card>
                        ...
                        <CardContent className="grid gap-2 text-sm">
                            ...
                            <p>
                                Keberangkatan:{' '}
                                {formatDate(invoice.booking.departure_date)}
                            </p>
                            <p>
                                {invoice.booking.participant_count} peserta x{' '}
                                {formatCurrency(
                                    Number(invoice.booking.package.price),
                                )}
                            </p>
                        </CardContent>
                    </Card>
                ...
                <Card>
                    ...
                    <CardContent className="grid gap-3">
                        ...
                        <p className="text-muted-foreground text-xs">
                            Terbit {formatDate(invoice.issued_date)} - jatuh
                            tempo{' '}
                            {invoice.due_date
